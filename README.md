...
これらは local 側の入出力すべてで除外される:

- `upload`: ローカルに存在してもアップロードされない
- `download --mirror`: ローカルにあっても削除候補に上がらない
- `download`: リモートに同名のパスがあってもローカルには書き込まれない

リモート側のリスト/読み出しには一切フィルタを掛けないため、`upload --mirror` ではリモートにこれらのパスがあれば削除候補に入る（local が source として持っていないため）。

ローカルの `.gitignore` は以下の場面でのみ適用される:

- `upload` 時の同期対象列挙: ignored ファイルは **アップロードしない**
- `download --mirror` 時の削除判定: ignored パスに一致する **ローカル側** のファイル/ディレクトリは **削除しない**
- `download` 時の書き込み判定には **影響しない**（リモートに ignored パスのファイルがあれば忠実に取得する）
- `upload --mirror` 時の削除判定には **影響しない**（ローカルに無いリモートのファイル/ディレクトリは ALWAYS_EXCLUDE 以外すべて削除される）

## 注意

- `--mirror` 指定時は対向の未同期変更が失われる可能性がある。実行前に `--dry-run` で計画を確認すること
- 特に `upload --mirror` はローカルの `.gitignore` を **削除判定では参照しない**。ローカルに存在しないファイル/ディレクトリは ALWAYS_EXCLUDE 以外すべてリモートから削除されるので、ワークスペース上だけで作業中のファイルなどは事前に退避するか `--mirror` を付けずに実行すること
